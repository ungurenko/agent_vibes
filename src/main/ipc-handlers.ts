...
const MAX_ALLOWED_IMAGE_PATHS = 256
const ALLOWED_IMAGE_PATH_TTL_MS = 6 * 60 * 60 * 1000

/** Paths selected via native dialog — allowed for getImageDataUrl */
const allowedImagePaths = new Map<string, number>()
...
function pruneAllowedImagePaths(): void {
  const now = Date.now()

  for (const [p, lastTouchedAt] of allowedImagePaths) {
    if (now - lastTouchedAt > ALLOWED_IMAGE_PATH_TTL_MS) {
      allowedImagePaths.delete(p)
    }
  }

  while (allowedImagePaths.size > MAX_ALLOWED_IMAGE_PATHS) {
    const oldest = allowedImagePaths.keys().next().value
    if (!oldest) break
    allowedImagePaths.delete(oldest)
  }
}

function rememberAllowedImagePath(filePath: string): void {
  const resolved = path.resolve(filePath)
  // Move existing path to the end to preserve LRU-like order.
  allowedImagePaths.delete(resolved)
  allowedImagePaths.set(resolved, Date.now())
  pruneAllowedImagePaths()
}
...
function isPathAllowedForImageRead(filePath: string): boolean {
  ...
  if (isPathInsideDir(resolved, getTempImagesDir())) return true
  // Allow paths that were selected via native dialog
  if (allowedImagePaths.has(resolved)) {
    rememberAllowedImagePath(resolved)
    return true
  }
  return false
}
...
export function registerIpcHandlers(mainWindow: BrowserWindow): void {
  ...
  ipcMain.handle('data:clearHistory', () => {
    ...
    // Drop temporary allow-list entries for user-selected images.
  ...
  ipcMain.handle('fs:pathExists', (_event, p: string) => {
    ...
    if (!isPathInsideDir(resolved, homeDir) && !isPathInsideDir(resolved, tempDir)) {
      return false
    }
  ...
  ipcMain.handle('dialog:selectImages', async () => {
    ...
    return result.filePaths.map((filePath) => {
      rememberAllowedImagePath(filePath)
      return {
  ...
  ipcMain.handle('fs:revokeImagePath', (_event, filePath: string) => {
    const resolved = path.resolve(filePath)
    allowedImagePaths.delete(resolved)
  })
  ...
  ipcMain.on('cli:login', (event) => {
    ...
    loginProcess = spawn('claude', ['login'], {
      env: getSafeEnv()
    })
    ...
    const timeout = setTimeout(() => {
      if (loginProcess) {
        loginProcess.kill()
        loginProcess = null
        event.sender.send('cli:login:complete', {
          success: false,
    ...
    loginProcess.on('close', (code) => {
      ...
      event.sender.send('cli:login:complete', {
        success: code === 0,
        error: code === 0 ? undefined : `Команда завершилась с кодом ${code}`
      })
    })
    ...
    loginProcess.on('error', (err) => {
      ...
      event.sender.send('cli:login:complete', { success: false, error: err.message })
    })
  })
}
