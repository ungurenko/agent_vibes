...
function loadMeta(): SessionMeta[] {
  try {
    ...
    const parsed = JSON.parse(raw) as Array<SessionMeta & { projectDir?: unknown }>
    return parsed
      .filter((entry) => entry && typeof entry === 'object')
      .map((entry) => ({
        ...entry,
        projectDir: typeof entry.projectDir === 'string' ? entry.projectDir : null
      }))
  } catch (err) {
...
export function saveSession(
  id: string,
  data: SessionData & { title?: string; projectName?: string; projectDir?: string | null }
): void {
  ...
  const metaEntry: SessionMeta = {
    ...
    projectDir: data.projectDir !== undefined ? data.projectDir : (idx >= 0 ? meta[idx].projectDir : null),
