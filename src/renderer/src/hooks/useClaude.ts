...
export function useClaude() {
  ...
  const sendMessage = useCallback(
    (prompt: string, projectDir: string, attachedImages?: AttachedImage[], model?: string, systemPrompt?: string, isPlanMode?: boolean, skillsContent?: string) => {
      ...
      const finalSystemPrompt = effectiveSystemPrompt || undefined
      ...
      setStatus('thinking')
      window.claude.execute(cliPrompt, projectDir, sessionId || undefined, model, finalSystemPrompt)
    },
    [sessionId]
  )
  ...
  const switchSession = useCallback(
    async (newAppSessionId: string | null) => {
      ...
      if (newAppSessionId && sessionsCacheRef.current.has(newAppSessionId)) {
        ...
        if (switchRequestIdRef.current !== requestId || appSessionIdRef.current !== newAppSessionId) {
          return
        }
